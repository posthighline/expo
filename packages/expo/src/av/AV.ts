...
/**
 * A decorator that adds common playback methods to A/V classes and returns a class that implements
 * the `Playback` interface.
 */
export function playback<T extends AV, A extends any[]>(
  AVClass: Constructor<T, A>
): Constructor<T & Playback, A> {
  // NOTE: This type assertion is on its own line since Prettier incorrectly removes the parentheses
  // around it when inlined (https://github.com/prettier/prettier/issues/5073)
  let _AVClass = AVClass as Constructor<AV, A>;
  return class extends _AVClass implements Playback {
